...
To generate a flamegraph from this result, follow [this tutorial](../flame-graphs#create-a-flame-graph-with-system-perf-tools)
from step 6.

Because `perf` output is not a Node.js specific tool, it might have issues with how JavaScript code is optimized in
Node.js. See [perf output issues](../flame-graphs#perf-output-issues) for a
further reference.
