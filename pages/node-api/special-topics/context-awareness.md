...
Each Worker Thread operates within its own environment which is also referred to as a context. The context is available to each Node-API function as an [`napi_env`](https://nodejs.org/api/n-api.html#napi_env) value.
...
Add-ons loaded in worker threads are unloaded when the last thread that has them loaded terminates (add-ons loaded on the main Node.js thread persist for the lifetime of the process.) To prevent memory leaks, any memory your native add-on has allocated _must_ be freed when your native add-on is unloaded.
...
[Environment Life Cycle APIs](https://nodejs.org/api/n-api.html#environment-life-cycle-apis) Node.js documentation for `napi_set_instance_data` and `napi_get_instance_data`.
...
[Cleanup on exit of the current Node.js instance](https://nodejs.org/api/n-api.html#cleanup-on-exit-of-the-current-nodejs-instance) Node.js documentation for `napi_add_env_cleanup_hook` and `napi_remove_env_cleanup_hook`.
